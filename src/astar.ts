import type { Coord, HeuristicType } from './types'
...
export type AStarPhase = 'searching' | 'reconstructing' | 'finished'
...
interface HeapEntry {
  ...
  y: number
  f: number
  h: number      // tiebreak: prefer smaller h (closer to goal)
  g: number      // staleness check: entry is stale if g !== bestG[key]
}

/**
 * Binary min-heap keyed by (f, h).
 * Uses lazy deletion: when g is improved we push a fresh entry; older entries
...
class MinHeap {
  ...
  snapshotCoords(): Coord[] {
    // Deduplicate: lazy-deletion means the heap can hold multiple stale entries
    // for the same cell; the UI only needs unique coords.
...
export function* aStar(
  ...
  const openMembership = new Uint8Array(ROWS * COLS)  // for openSet snapshot hints

  const h0 = heuristic(start, end, heuristicType)
  bestG[startKey] = 0
  open.push({ x: start.x, y: start.y, f: h0, h: h0, g: 0 })
  openMembership[startKey] = 1
  ...
  while (open.size > 0) {
    ...
    for (const d of DIRS) {
      ...
      if (tentativeG < bestG[nk]) {
        ...
        parentX[nk] = entry.x
        parentY[nk] = entry.y
        const nh = heuristic({ x: nx, y: ny }, end, heuristicType)
        open.push({ x: nx, y: ny, f: tentativeG + nh, h: nh, g: tentativeG })
        openMembership[nk] = 1
      }
    }

...
import type { Cell, Coord, HeuristicType } from '../types'

export function useAStarRunner(
  cells: Ref<Cell[][]>,
  start: Ref<Coord>,
  end: Ref<Coord>,
  ...
  let generator: Generator<AStarState> | null = null
  ...
  function applyState(state: AStarState) {
    for (const row of cells.value) {
      for (const cell of row) {
        cell.state = 'idle'
      }
  ...
  function initGenerator() {
    ...
    pathCost.value = 0
    elapsedMs.value = 0
    generator = aStar(start.value, end.value, heuristicType.value, getWalls, getWeight)
  }
  ...
  // Ensure cells are visible immediately (no fog for A*)
  revealAll()
